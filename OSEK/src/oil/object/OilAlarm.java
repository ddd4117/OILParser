...
public class OilAlarm extends OilObject {
  ...
  public void addAttribute(String name, String value)
    ...
    case "ACTION":
      ...
      if (value.contains("SETEVENT")) {
        setAction(OilAlarm.alarmAction.toEnum("SETEVENT"));
        action.event = splitList[1];
      }
      else if (value.contains("ACTIVATETASK")) {
        setAction(OilAlarm.alarmAction.toEnum("ACTIVATETASK"));
        action.task = splitList[1];
      }
      else {
        setAction(OilAlarm.alarmAction.toEnum("ALARMCALLBACK"));
        action.alarmCallBack = splitList[1];
      }
      break;
    case "AUTOSTART":
      setAutostart(value.contains("TRUE"));
      if (this.autostart) {
        // if autostart is true, set ALARMTIME, CYCLETIME, APPMODE.
        splitList = value.split("=");
        setAlarmTime(Integer.parseInt(splitList[1]));
      }
      break;
    case "CYCLETIME":
